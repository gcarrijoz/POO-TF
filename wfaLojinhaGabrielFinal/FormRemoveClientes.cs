...
{
    public partial class FormRemoveClientes : Form
    {
        public FormRemoveClientes()
        {
            InitializeComponent();
        }

        public FormRemoveClientes(DataGridViewRow linha)
        {
            InitializeComponent();
            if(linha != null)
            {
        ...
        private void btCadastrarCliente_Click(object sender, EventArgs e)
        {
            if(ValidaInformacao() == true)
            {
                ...
                string stringSql = $"DELETE FROM clientes WHERE id_cliente = '{idCliente}'";
                ...
                MessageBox.Show("Cliente Excluído com Sucesso! ", Text, MessageBoxButtons.OK, MessageBoxIcon.Information);
                LimparTextBox();
